  ...
  Entity* Physics::rayCast(const double &angle, const double &width) { // TODO: Refactor
    ...
    for (std::vector<Entity*>::iterator entity = entities.begin(); entity != entities.end(); ++entity) {
      ...
      if (angle > 0) {
        ...
        std::pair<double, double> bottomLeftCorner = Vision::Perspective::virtualToReal((*entity)->getBlob()->getMinX() - radiusVectorX, (*entity)->getBlob()->getMaxY() - radiusVectorY, (*entity)->getBlob()->getCameraID());
        double bottomLeftAngle = -atan2(bottomLeftCorner.first, bottomLeftCorner.second);
        std::pair<double, double> topRightCorner = Vision::Perspective::virtualToReal((*entity)->getBlob()->getMaxX() + radiusVectorX, (*entity)->getBlob()->getMinY() + radiusVectorY, (*entity)->getBlob()->getCameraID());
      ...
      // center of the camera frame
        ...
        // Calculate corresponding blob corner angles // TODO: Correct this method - it is not exact for top corners
        std::pair<double, double> topLeftCorner = Vision::Perspective::virtualToReal((*entity)->getBlob()->getMinX() - radiusVectorX, (*entity)->getBlob()->getMinY() - radiusVectorY, (*entity)->getBlob()->getCameraID());
        double topLeftAngle = -atan2(topLeftCorner.first, topLeftCorner.second);
        std::pair<double, double> bottomRightCorner = Vision::Perspective::virtualToReal((*entity)->getBlob()->getMaxX() + radiusVectorX, (*entity)->getBlob()->getMaxY() + radiusVectorY, (*entity)->getBlob()->getCameraID());
